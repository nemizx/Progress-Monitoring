...
export default forwardRef(function DaysReportPanel({
  projectId,
  subProjectId,
  selectedDate,
  isDateLocked = false,
  ...
  // Fetch days reports for the selected date
  const { data: selectedDateEntries = [], isLoading: dateLoading } = useQuery({
    queryKey: ['days-report-date', projectId, subProjectId, selectedDate],
    queryFn: () => base44.entities.DaysReport.filter({
  ...
  const performSave = useCallback(async () => {
    if (deletedIds.length > 0) {
      await Promise.all(deletedIds.map(id => base44.entities.DaysReport.delete(id)));
    }
    ...
    await Promise.all(validRows.map(row => {
      ...
      if (row.id && !row.id.startsWith('temp_')) {
        return base44.entities.DaysReport.update(row.id, payload);
      }
      return base44.entities.DaysReport.create(payload);
    }));
  ...
  const validate = useCallback(() => {
    ...
    if (hasEmptyDescription && rows.length > 1) {
      return "Day's Report: ensure all rows have a description.";
    }
    return null;
  }, [rows]);
  ...
  useDprPanelRef(ref, {
    ...
    save: async () => {
      ...
      await performSave();
      queryClient.invalidateQueries({ queryKey: ['days-report-date', projectId] });
      setLoadedDate(null);
      setDeletedIds([]);
    },
  });
  ...
  if (dateLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-sm text-muted-foreground font-sans">Loading day's report...</span>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        {isDateLocked && (
          <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">
            DPR is locked for this date. Day's report cannot be changed.
          </div>
        )}

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <StatCard title="Report Items" value={validEntriesCount} icon={ClipboardList} />
                    ...
                    <div className="flex items-center gap-1 select-none">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[220px] text-center font-sans font-normal normal-case">
                          Details of the work completed, weather delays, safety, or key events of the day.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  <th className="text-left p-3 font-semibold text-xs text-muted-foreground uppercase w-[300px]">
                    <div className="flex items-center gap-1 select-none">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[220px] text-center font-sans font-normal normal-case">
                          Optional comments, follow-ups, or notes related to this item.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  ...
                  <tr key={row.id} className="border-b hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-3">
                      <Textarea
                        ...
                        placeholder="e.g. Concreting of 1st floor slab completed using M25 grade concrete."
                    ...
                    <td className="p-3 align-top">
                      <Input
                        ...
                        placeholder="e.g. Delayed by 2 hours due to heavy rain."
                        disabled={isDateLocked}
