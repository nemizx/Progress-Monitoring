...
export default forwardRef(function NextDaysPlansPanel({
  projectId,
  subProjectId,
  selectedDate,
  isDateLocked = false,
  ...
  // Fetch next day's plans for the selected date
  const { data: selectedDateEntries = [], isLoading: dateLoading } = useQuery({
    queryKey: ['next-days-plans-date', projectId, subProjectId, selectedDate],
    queryFn: () => base44.entities.NextDaysPlan.filter({
  ...
  const performSave = useCallback(async () => {
    if (deletedIds.length > 0) {
      await Promise.all(deletedIds.map(id => base44.entities.NextDaysPlan.delete(id)));
    }
    ...
    await Promise.all(validRows.map(row => {
      ...
      if (row.id && !row.id.startsWith('temp_')) {
        return base44.entities.NextDaysPlan.update(row.id, payload);
      }
      return base44.entities.NextDaysPlan.create(payload);
    }));
  }, [deletedIds, rows, projectId, subProjectId, selectedDate]);

  const validate = useCallback(() => {
    const hasEmptyDescription = rows.some(r => !r.description.trim() && (r.unit.trim() || r.quantity.trim()));
    if (hasEmptyDescription) {
      return "Next Day's Plan: description is required when unit or quantity is filled.";
    }
    return null;
  }, [rows]);
  ...
  useDprPanelRef(ref, {
    ...
    save: async () => {
      ...
      await performSave();
      queryClient.invalidateQueries({ queryKey: ['next-days-plans-date', projectId] });
      setLoadedDate(null);
      setDeletedIds([]);
    },
  });
  ...
  if (dateLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-sm text-muted-foreground font-sans">Loading next day's plans...</span>
      </div>
    );
  }
      ...
      <div className="space-y-4">
        {isDateLocked && (
          <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-4 py-2">
            DPR is locked for this date. Next day's plan cannot be changed.
          </div>
        )}

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <StatCard title="Planned Items" value={validEntriesCount} icon={ClipboardList} />
                    ...
                    <div className="flex items-center gap-1 select-none">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[220px] text-center font-sans font-normal normal-case">
                          Details of the work planned to be executed tomorrow. This field is mandatory.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  <th className="text-left p-3 font-semibold text-xs text-muted-foreground uppercase w-[180px]">
                    <div className="flex items-center gap-1 select-none">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[220px] text-center font-sans font-normal normal-case">
                          Optional measurement unit (e.g. Cum, Sqm, MT, Rmt).
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  <th className="text-right p-3 font-semibold text-xs text-muted-foreground uppercase w-[180px]">
                    <div className="flex items-center justify-end gap-1 select-none">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-[220px] text-center font-sans font-normal normal-case">
                          Optional target quantity of work to be accomplished tomorrow.
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  ...
                  <tr key={row.id} className="border-b hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-3">
                      <Textarea
                        ...
                        placeholder="e.g. Concreting of foundation columns for Block A."
                    ...
                    <td className="p-3 align-top">
                      <Input
                        ...
                        placeholder="e.g. 150"
                        disabled={isDateLocked}
                        className="h-8.5 text-xs bg-background w-full text-right"
                      />
                    </td>
