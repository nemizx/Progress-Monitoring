...
export default function QualityManagement() {
  ...
  const { data: inspections = [] } = useQuery({
    ...
    queryFn: () => projectId
      ? base44.entities.QualityInspection.filter({ project_id: projectId }, '-created_date', 200)
      : Promise.resolve([]),
    enabled: !!projectId,
  });
  ...
  const scopedInspections = isReady ? inspections : [];
  const filtered = statusFilter === 'all' ? scopedInspections : scopedInspections.filter(i => i.status === statusFilter);

  const passedCount = scopedInspections.filter(i => i.status === 'passed').length;
  const failedCount = scopedInspections.filter(i => i.status === 'failed').length;
  const avgScore = scopedInspections.length > 0 ? Math.round(scopedInspections.reduce((s, i) => s + (i.compliance_score || 0), 0) / scopedInspections.length) : 0;
  const criticalIssues = scopedInspections.filter(i => i.severity === 'critical' && i.status !== 'passed').length;

  const statusData = STATUSES.map(s => ({ name: s.replace(/_/g, ' '), value: scopedInspections.filter(i => i.status === s).length })).filter(d => d.value > 0);
  ...
  const openAddDialog = () => {
    setForm({
      ...
      title: '', inspection_type: 'general', status: 'scheduled',
      inspector_name: '', inspection_date: '', findings: '', severity: 'minor',
      corrective_action: '', compliance_score: 80,
    });
    setShowAdd(true);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Track inspections, compliance, and quality control
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Inspections" value={scopedInspections.length} icon={ShieldCheck} />
